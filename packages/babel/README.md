...
The `include`, `exclude`, and `extensions` options are ignored when using `getBabelOutputPlugin` and `createBabelOutputPluginFactory`, which will produce warnings. There are a few more points to note that users should be aware of.
